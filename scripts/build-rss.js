...
const siteUrl = 'https://rulebox.io/blog'
...
const feed = new Feed({
  ...
  copyright: `All rights reserved ${new Date().getFullYear()}, rulebox  Data Intelligence`,
  feedLinks: {
  ...
  author: {
    name: 'Rulebox.io',
    link: 'https://twitter.com/Rulebox_io',
  },
})
...
getAllPosts().forEach(({ link, module: { meta, default: Content } }) => {
  ...
  const postText = `<p><em>(The post <a href="${siteUrl + link}">${
    meta.title
  }</a> appeared first on <a href="${siteUrl}">rulebox Blog</a>.)</em></p>`
  feed.addItem({
    title: meta.title,
    id: meta.title,
    link,
